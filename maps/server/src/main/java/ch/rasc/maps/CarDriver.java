...
public class CarDriver {
	...
	public CarDriver(CentrifugoServerApiClient centrifugoServerApiClient, ObjectMapper objectMapper)
			throws IOException {
	...
	private static List<LatLng> readLatLng(String resource) throws IOException {
		...
		try (InputStream is = cp.getInputStream()) {
			...
			route = Arrays.stream(content.split("\n")).map(LatLng::new).collect(Collectors.toList());
		}
		return route;
	}
	...
	public void driveBlueCar() {
		...
		this.centrifugoServerApiClient.publication()
			.publish(b -> b.channel("map.blue").data(this.objectMapper.convertValue(latLng, Map.class)));
	}
	...
	public void driveRedCar() {
		...
		this.centrifugoServerApiClient.publication()
			.publish(b -> b.channel("map.red").data(this.objectMapper.convertValue(latLng, Map.class)));
	}
